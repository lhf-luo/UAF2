...
std::map<std::string, std::vector<StructType*>> CallGraphPass::StructGroups;
...
std::string CallGraphPass::SingleType2String (Type *Ty) {
	...
	if (IsCompositeType(Ty)) {
		...
		if (StructType* STy = dyn_cast<StructType>(Ty)) { 			
			TypeName = STy->getStructName().str();
			...
			*/
            ...
            uint64_t Num = ATy->getNumElements();
            std::string NumStr = std::to_string(Num);

            std::string ElemName = SingleType2String(ElemTy);

            if (ElemName == "NAMESET") {
                std::set<std::string> ElemNames = std::move(TypeNameSet); 
                TypeNameSet.clear();
                for (const std::string& ename : ElemNames) { 
                    TypeNameSet.insert("array[" + NumStr + "x" + ename + "]");
                }
                TypeName = "NAMESET";
            } else {
                TypeName = "array[" + NumStr + "x" + ElemName + "]";
            }
	 	}
...
bool CallGraphPass::analyzeBitCast(BitCastInst *BC) {
    Type *DestTy = BC->getDestTy();
    if (PointerType *PT = dyn_cast<PointerType>(DestTy)) {
        Type *ElTy = BC->getDestTy()->getPointerElementType();
        if (ElTy->isFunctionTy()) {
            return true;
        }

        else if (ElTy->isPointerTy()) {
            Type *InnerTy = ElTy->getPointerElementType();
            if (InnerTy->isFunctionTy()) {
                return true;
            }
        }
    }
...
void CallGraphPass::GlobalVariableAnalysis(GlobalVariable *GV, Constant *Ini){
	if(GV->getName().contains("pf_profile")){
		OP<<"pf_profile";
	}
...
bool CallGraphPass::IsDegradedType(Type* Ty) {
	while (PointerType *PTy = dyn_cast<PointerType>(Ty)) {
		Ty = PTy->getPointerElementType();
	}

	if (StructType* SSTy = dyn_cast<StructType>(Ty)) {
        	
		return SSTy->isOpaque() || SSTy->getNumElements() == 0;
	}
    return false;
}

bool CallGraphPass::IsDegradedStructType(StructType* STy){
	for (Type* Ty : STy->elements()) {
		if(IsDegradedType(Ty)){
			return true;
		}
	}
	return false;
...
void CallGraphPass::constructStruct(){
	for (auto& Pair : StructGroups) {
    	std::string BaseName = Pair.first;
    	std::vector<StructType*>& Variants = Pair.second;

    	if (Variants.size()<2 ) continue;
    	unsigned NumElements = Variants[0]->getNumElements();
    
    	std::string PerfectID = "";

    	for (unsigned i = 0; i < NumElements; ++i) {
        	Type* BestFieldTy = nullptr;

        	for (StructType* Variant : Variants) {
            	if (i >= Variant->getNumElements()) continue; 
            	Type* CurrentFieldTy = Variant->getElementType(i);
            	if (!IsDegradedType(CurrentFieldTy)) {
                	BestFieldTy = CurrentFieldTy;
					break;
            	}
        	}
			// only one
			if (BestFieldTy) {
				PerfectID += SingleType2String(BestFieldTy); 
			} else {
				PerfectID += "unknown";
			}
    	}

		StructIDNameMap[PerfectID].insert(BaseName);
		if (BaseName.find("mlx5_ib_stage") != std::string::npos) {
        	errs() << "Merged ID for " << BaseName << ": " << PerfectID << "\n";
    	}
	}
}
...
list<std::string> CallGraphPass::MLTypeName2List(std::string MLTypeName) {
	...
	// Get number of layers: redundant?
	/*int LayerNum = 1;
	int i = 0;
...
bool CallGraphPass::FindCalleesWithSMLTA(CallInst *CI, FuncSet &FS) {
...
bool CallGraphPass::doFinalization(Module *M) {
	...
	for (auto CI : Ctx->CallSet) {
		FuncSet FS;
		for (auto F : Ctx->Callees[CI]) {
			if (F->getName().find("llvm") != std::string::npos) {
				Ctx->Callees.erase(CI);
				continue;
			}
			if (F->isDeclaration()) {
		...
		// TODO: reserve declare functions...
		if (FS.empty())
			continue;
		Ctx->Callees[CI] = FS;
	}

	for (auto pair : Ctx->Callees) {
		//TODO: why there are so many empty functions?
		/*
		errs() << "Call site: ";
		pair.getFirst()->getDebugLoc().print(errs());
		errs() << "\n";
		errs() << "Targets: \n";
		*/
		std::string FunctionName=pair.getFirst()->getFunction()->getName().str();
		if(FunctionName.find("__mlx5_ib_add")!=std::string::npos){
			for (auto func : pair.getSecond()) {
				errs() << func->getName().str() << "\n";
				//func->print(errs());
				Ctx->CalleeFuncSet.insert(func);
			}
		}
	}
	//errs() << "Number of identified functions :" << CalleeFuncSet.size() << "\n";
