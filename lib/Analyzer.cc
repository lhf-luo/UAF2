...
cl::opt<std::string> InputList(
    "input-list", cl::desc("Specify a file containing a list of input files (one per line)"));
...
char IterativeModulePass::PassID = 0;
...
int main(int argc, char **argv) {
	...
	if (!InputList.empty()) {
        std::ifstream in(InputList.c_str());
        if (!in.is_open()) {
            errs() << "Error: Cannot open input list file: " << InputList << "\n";
            return 1;
        }
        std::string line;
